...
save_state_dict=f"{model_files_dir}dict.pth"
...
def get_model(load_weights = True):
    ...
    m =nn.Sequential(ReCodeAlphabet(), deepsea_cpu)
    if load_weights:
        try:
            m.load_state_dict(torch.load(save_state_dict), strict=False)
            print("loaded weights")
        except:
            torch.save(m.state_dict(), save_state_dict)
    return m
...
def get_seqpred_model(load_weights = True):
    ...
    m=nn.Sequential(ReCodeAlphabet(), ConcatenateRC(), deepsea_cpu, AverageRC())
    if load_weights:
        try:
            m.load_state_dict(torch.load(save_state_dict), strict=False)
            print("loaded weights")
